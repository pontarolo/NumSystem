...
char *octal(char *value, TokenType base)
{
    ...
    char *result = (char *) calloc(calculate_digits(decimal(value, base), TOKEN_OCTAL),
                                   sizeof(char));
...
{
    FPoint number = break_str(value, ".");
    char *result = (char *) calloc(calculate_digits(decimal(value, base), TOKEN_BINARY),
                                   sizeof(char));
...
double decimal(char *value, TokenType base)
{
    ...
    return (number.hasDecimal) ? integer_sum + decimal_sum / pow(10, strlen(number.decimal - 1))
                               : integer_sum;
}
